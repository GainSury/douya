...
// This script will be injected into page 'https://www.douban.com/group/'
...
var statCounterTags = `
  <script type="text/javascript">
    var tags = {"Version": versionNumber};
    if (myProfile.id) {
...
function handleResponse(response) {
  ...
  for (var i = 0; i < response.topics.length; i++) {
    ...
    topicHTML += `
        ...
        <td class="td-tag">${tags.map(tag => `<label class="tag">${tag}</label>`).join('&nbsp;')}</td>
        <td class="td-reply">${topic.comments_count}</td>
        <td class="td-like">${topic.like_count}</td>
        <td class="td-group">
          <span><a href="${topic.group.url}" title="${topic.group.name}">${topic.group.name}</a></span>
        </td>
